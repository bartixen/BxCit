...
public class SkinsPlayerCommand implements CommandExecutor {

    Main plugin;

    SkinsDataManager data;

    static ArrayList<String> list_of_sellers = new ArrayList<>();
    static ArrayList<String> list_pkt = new ArrayList<>();

    public SkinsPlayerCommand(Main m) {
        plugin = m;
        ...
        if (sender.hasPermission("bxcit.commands.gracz") || sender.isOp()) {
            if (args.length == 1) {
                ...
                if (uuid != null) {
                    ...
                    list_of_sellers = (ArrayList<String>) data.getData().getStringList(uuid + ".lista_sprzedaz");
                    list_pkt = (ArrayList<String>) data.getData().getStringList(uuid + ".lista_pkt");
                    sender.sendMessage("§7Nick: §9" + nick);
                    sender.sendMessage("§7Punkty: §9" + pkt);
                    sender.sendMessage("§7Wplaty:");
                    if (list_pkt.size() > 0) {
                        for (String blacklist : list_pkt) {
                    ...
                    if (list_of_sellers.size() > 0) {
                        for (String blacklist : list_of_sellers) {
